...
alias oqproc {
  if (%oqnumo > $lines(%oqplik)) { oqprocend }
    ...
    .timerdelay 1 %oqdelay msg %oqkanal %oqk1 Pytanie %oqnrpyt z %oqpytania za %oqstaw punkty $+ : $+ %oqkpyt %oqq
    .timerpodp0 1 %oqdelay msg %oqkanal %oqk1 Podpowiedz:  %oqkpyt %oqpodp1
    inc %oqnumo 2
    inc %oqnump 2
    if (%oqodp == 1) {
      .timerpodp1a 1 $calc(%oqdelay + 8) set %oqstaw 2
    .timerpodp1b 1 $calc(%oqdelay + 8.1) msg %oqkanal %oqk1 Podpowiedz:  %oqkpyt %oqpodp2 }
    if (%oqodp == 1) {
      .timerpodp2a 1 $calc(%oqdelay + 14) set %oqstaw 1
    .timerpodp2b 1 $calc(%oqdelay + 14.1) msg %oqkanal %oqk1 Podpowiedz:  %oqkpyt %oqpodp3 }
...
alias -l pkt2 {
  if ((%oqpier != $nick) && (%oqpierial != $address($nick,1))) {
    ...
    if (%oqstawwait > 1) { msg %oqkanal 0,10 Extra  $+ 8,10 %oqstawwait $+ 0,10 punkty dla %oqg $+ 8,10 $+ $nick $+ 0,10 $+ %oqg Punktow: 8,10 $+ %oqsuma $+ 0,10 Pozycja: $+ 8,10 %oqplace $+ 0,10 %oqk1 }
    else { msg %oqkanal 0,10 Extra $+ 8,10 %oqstawwait $+ 0,10 punkt dla %oqg $+ 8,10 $+ $nick $+ 0,10 $+ %oqg Punktow: 8,10 $+ %oqsuma $+ 0,10 Pozycja: $+ 8,10 %oqplace $+ 0,10 %oqk1 }
    unset %oqa2
  }
}
#wait end
